...
static int tegra_wdt_probe(struct platform_device *pdev)
{
	...
	if (pdev->id == -1) {
		...
		wdt->miscdev.minor = WATCHDOG_MINOR; // force as WATCHDOG_MINOR instead of MISC_DYNAMIC_MINOR
		if (pdev->id == 0)
			wdt->miscdev.name = "watchdog";
		else if (pdev->id == 1)
	...
	dev_info(&pdev->dev, "tegra_wdt is registered as '%s' miscdev.minor=%d\n",wdt->miscdev.name,wdt->miscdev.minor);
